...
### 🆕 官方 MCP 实测对照:同花顺 vs 通达信(2026-09-12)

**结论**:两家券商/资讯厂都已提供官方 MCP · 都支持用户自助申请 key · 走的正是 hunter「玩法 ②:自接 MCP」这条路 · 我们代码零密钥。

| | 同花顺(HiThink) | 通达信(TDX) |
|---|---|---|
| 官方 MCP 端点 | `https://fuyao.aicubes.cn/mcp/*` · **6 个**(A股/指数/基金/期货/期权/元数据) | `https://txmcp.tdx.com.cn:3001/clawmcp` · **1 个** |
| 鉴权 | `X-api-key: <key>` | `Authorization: Bearer TDX-<key>` |
| Key 获取 | 文档站登录三步签发 · 未见收费 | 商城「积分和 Key 管理」自助创建 · **付费积分制** |
| 独家能力 | ⭐ **全市场 10 年日 K · 172MB Parquet 一次拉全** · 龙虎榜机构/游资分开 63 条 | ⭐ **资讯/公告/研报/宏观** 四工具 · 自然语言选股 · K 线含复权 |
| 数据准确性 | ✅ 茅台 1275.16(三方对账一致) | ✅ 茅台 1275.16(三方对账一致) |
| 限流 | 1/4/8 并发共 120 请求零 429 | 积分扣费 · 单次成本待确认 |
| **商用/转售条款** | ⚠️ **387KB 文档零命中** | ⚠️ **未见** |

**接入姿势**(与「玩法 ②」完全一致):
1. 用户在数据源设置里填自己的 key(明显位置)
2. 后端按用户选的源挂对应 header · 我们代码零 key
3. 用户没填 key → 明确报错 + 官方申请入口链接 · **不用我们的 key 兜底**

**⚠️ 铁律**:我们那把 key 只用于开发调试 · 不进仓库 / 不进镜像 / 不替用户调 —— 一旦替用户取数即构成转售数据 · 商用前请自行向官方确认服务条款。

**完整实测报告**(证据链 · 401 鉴权验证 · JSON-RPC 抓包):[`doc/data-source/2026-09-12_ths-vs-tdx-mcp.md`](./doc/data-source/2026-09-12_ths-vs-tdx-mcp.md)
