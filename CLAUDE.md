...
14. **评分换成用户的五项 500 分制(2026-09-12 深夜,替换第 13 条的 10 分制)**:形态(原 10 分制当第 1 项)/ 量价配合(63 天内涨放量跌缩量的净次数)/
    ...
    (a) **盈亏比的「预期涨幅」是 Claude 定的口径**(底部量度目标 = 枢轴 × (1 + 首次收缩深度)),用户没给目标价;事前验证 29 笔里 26 笔这一项是 D,
    几乎不区分,总分实际上限 400 —— 改口径先和用户对,别自己换成 52 周高点之类。
    ...
    重跑全年:+3.23%(回撤 -4.97%,19 笔卖出,胜率 42%,盈亏比 1.79,夏普 0.78,敞口均值 16% / 最大 55%),D 级挡下 23 天次;
    开仓 S 1 / A 3 / B 4 / C 12。同样:样本太小,别拿它当结论。
