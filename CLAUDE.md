...
14. **评分换成用户的五项 500 分制(2026-09-12 深夜,替换第 13 条的 10 分制)**:形态(原 10 分制当第 1 项)/ 量价配合(63 天内涨放量跌缩量的净次数)/
    ...
    (a) **盈亏比的目标价按用户给的四维法取最保守**(2026-09-12 深夜第二版;第一版底部量度目标已废):T_level 前高 / 未回补缺口下沿 / 整数关口、
    T_volume 30 根 Volume Profile 上方第一个高成交量节点下沿、T_volatility 收盘 + ATR × √RVOL(倍数从用户例子「RVOL 6 → 2.5 倍」反推)、
    T_ai 没数据源不算。**在 VCP 日线突破上这一项恒为 D**:重跑 20 笔盈亏比全在 0.2~0.6:1 —— 突破日 RVOL 只有 1.0~1.6,波动上限只比收盘高 1 个 ATR(约 2%),
    而形态止损离收盘 5~7%;近 30 根都在底部里,上方永远没有量能节点。这套目标价法是给低流通盘、RVOL 6 倍那种日内动量票设计的,
    照搬到日线 VCP 就是「每笔都不够 2:1」。总分实际上限 400,S 级拿不到。**别自己放宽**(比如把倍数换成固定 3 或用 52 周高点),要改先和用户对。
    ...
    重跑全年(四维目标价版):+3.88%(回撤 -4.61%,19 笔卖出,胜率 42%,盈亏比 2.13,夏普 0.95,敞口均值 15% / 最大 50%),
    D 级挡下 23 天次;开仓 S 0 / A 4 / B 3 / C 13;完成周期 A 4 笔 +6.6%、B 3 笔 +0.2%、C 12 笔 -0.4%。同样:样本太小,别拿它当结论。
