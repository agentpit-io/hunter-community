...
14. **评分换成用户的五项 500 分制(2026-09-12 深夜,替换第 13 条的 10 分制)**:形态(原 10 分制当第 1 项)/ 量价配合(63 天内涨放量跌缩量的净次数)/
    ...
    (a) **盈亏比按 R 倍数**(2026-09-12 深夜第三版,用户拍板「直接用 R 倍数目标」,具体口径 Claude 定):R = 收盘 − 止损,
    目标 = 上方最近的显著阻力(近 252 根不含今天的最高价 = 52 周高点、未回补缺口下沿,取近的;离收盘不足 0.5 ATR 不算),
    已创新高就用底部量度目标(枢轴 × (1 + 首次收缩深度)),盈亏比 = 空间 ÷ R。前两版都废了:第一版量度目标 26/29 笔 D,
    第二版四维取最保守(整数关口 / ATR × √RVOL / 30 根量能节点)20/20 笔 D —— 目标被压在 1 个 ATR 内。四维那几个函数保留着没删。
    **第三版在这批票上仍然 20/20 笔 D**(空间 0.2~1.6R):原因是结构性的 —— 筛选器选出的 VCP 底部很浅(首次收缩多在 3~10%),
    枢轴常比 52 周高点低几个百分点,而形态止损离收盘 5~7%,「到阻力的空间」天然小于一个止损距离。要让这一项有区分度,
    得动的是止损尺度或候选池,不是目标价算法;**别再换第四种目标价口径**,先把这个结论给用户。
    ...
    重跑全年(第三版 R 倍数,与第二版逐笔相同,因为这一项两版都是 D):+3.88%(回撤 -4.61%,19 笔卖出,胜率 42%,盈亏比 2.13,
    夏普 0.95,敞口均值 15% / 最大 50%),D 级挡下 23 天次;开仓 S 0 / A 4 / B 3 / C 13;完成周期 A 4 笔 +6.6%、B 3 笔 +0.2%、
    C 12 笔 -0.4%。同样:样本太小,别拿它当结论。
